...
The store lives at .claude/memory/memory.db - gitignored, like the markdown it
...
CLI:
    ...
    memory.py --export [--out .claude/memory/export.md]
    memory.py --stats
...
def export_markdown(conn: sqlite3.Connection) -> str:
    ...
    out = [f"# Project memory export ({now()})", "",
           (f"Store: `.claude/memory/memory.db` - {c['lesson']} lessons, "
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--db", default="", help="store path (default .claude/memory/memory.db)")
